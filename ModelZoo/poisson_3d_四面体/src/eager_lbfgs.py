...
def lbfgs(opfunc, x, state, maxIter = 100, learningRate = 1, do_verbose = True):
  ...
  train_info = "train_lbfgs_info.txt"
  open("train_lbfgs_info.txt", 'w').close()
  ...
  while nIter < maxIter:
    ...
    if state.nIter == 1:
      tmp1 = abs(g)
      t = min(1, 1/mnp.sum(tmp1)) * learningRate
    else:
      t = learningRate
    ...
    if abs(f-f_old) < tolX:
      ...
      print('function value changing less than tolX'+str(abs(f-f_old)))
      break

    if do_verbose:
      if nIter % 10 == 0:
        info = "Step %3d loss %.2e "%(nIter, f.asnumpy())
        print(info)
